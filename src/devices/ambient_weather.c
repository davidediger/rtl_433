...
static float get_os_temperature(unsigned char *message, unsigned int sensor_id) {
...
static unsigned int get_os_humidity(unsigned char *message, unsigned int sensor_id) {
 // sensor ID included to support sensors with temp in different position
 int humidity = 0;
...
static unsigned int get_os_rollingcode(unsigned char *message, unsigned int sensor_id){
   int rc = 0;
...
static unsigned long total(const unsigned char* d){
  unsigned long val = 0;
  if ( (d[1]&0x0F) == 0 ){
...
static int ambient_weather_parser(uint8_t bb[BITBUF_ROWS][BITBUF_COLS], int16_t bits_per_row[BITBUF_ROWS]) {
...
static int ambient_weather_callback (uint8_t bb[BITBUF_ROWS][BITBUF_COLS], int16_t bits_per_row[BITBUF_ROWS]) {
